...
async function list(conversationId, { limit, before }) {
  ...
  const [rows] = await pool.execute(
    `SELECT m.id, m.sender_id AS senderId,
            ...
            CASE WHEN m.deleted_at IS NULL THEN a.id ELSE NULL END AS attachmentId,
            CASE WHEN m.deleted_at IS NULL THEN a.original_name ELSE NULL END AS attachmentName,
            CASE WHEN m.deleted_at IS NULL THEN a.mime_type ELSE NULL END AS attachmentMimeType,
            CASE WHEN m.deleted_at IS NULL THEN a.file_size ELSE NULL END AS attachmentSize
     FROM messages m
...
async function create({ id, conversationId, senderId, content, messageType, attachment, createdAt }) {
...
async function softDelete(messageId) {
  await pool.execute(
    `UPDATE messages
     SET content = NULL, deleted_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP
     WHERE id = ? AND deleted_at IS NULL`,
    [messageId]
  );
}
...
async function getAttachmentForUser(attachmentId, userId) {
  const [rows] = await pool.execute(
    `SELECT a.id, a.original_name AS originalName, a.file_path AS filePath,
            a.mime_type AS mimeType, a.file_size AS fileSize,
            m.deleted_at AS messageDeletedAt
     FROM message_attachments a
     JOIN messages m ON m.id = a.message_id
     JOIN conversation_participants cp ON cp.conversation_id = m.conversation_id
     WHERE a.id = ? AND cp.user_id = ? AND cp.left_at IS NULL
     LIMIT 1`,
    [attachmentId, userId]
  );
  return rows[0] || null;
